...
import com.priitlaht.maurus.backend.domain.SocialUserConnection;
...
public class CustomSocialConnectionRepository implements ConnectionRepository {
  ...
  public CustomSocialConnectionRepository(String userId, SocialUserConnectionRepository repository, ConnectionFactoryLocator locator) {
    this.userId = userId;
    this.socialUserConnectionRepository = repository;
  ...
  public void updateConnection(Connection<?> connection) {
    SocialUserConnection socialUserConnection = socialUserConnectionRepository
      .findOneByUserIdAndProviderIdAndProviderUserId(userId, connection.getKey().getProviderId(), connection.getKey().getProviderUserId());
  ...
  public void removeConnection(ConnectionKey connectionKey) {
    socialUserConnectionRepository
      .deleteByUserIdAndProviderIdAndProviderUserId(userId, connectionKey.getProviderId(), connectionKey.getProviderUserId());
  }

  private Double getNewMaxRank(String providerId) {
    List<SocialUserConnection> socialUserConnections =
      socialUserConnectionRepository.findAllByUserIdAndProviderIdOrderByRankAsc(userId, providerId);
    return socialUserConnections.stream().mapToDouble(SocialUserConnection::getRank).max().orElse(0D) + 1D;
  }
  ...
  private List<Connection<?>> providerUserIdsToConnections(String providerId, List<String> providerUserIds) {
    List<SocialUserConnection> socialUserConnections =
      socialUserConnectionRepository.findAllByUserIdAndProviderIdAndProviderUserIdIn(userId, providerId, providerUserIds);
    return socialUserConnectionsToConnections(socialUserConnections);
  }
  ...
  private Connection<?> socialUserConnectionToConnection(SocialUserConnection socialUserConnection) {
    ConnectionData connectionData = new ConnectionData(socialUserConnection.getProviderId(), socialUserConnection.getProviderUserId(),
      socialUserConnection.getDisplayName(), socialUserConnection.getProtileURL(), socialUserConnection.getImageURL(),
